...
const selected=new Set(); let advancedResults=[]; let bulkMode=true;
...
function decorateRows(){qa('#applicantTbody tr.applicant-row').forEach(tr=>{const onclick=tr.getAttribute('onclick')||'';const m=onclick.match(/viewApplicant\('([^']+)'\)/);if(!m)return;const id=m[1];let td=tr.querySelector('.bulk-select-cell');if(!td){td=document.createElement('td');td.className='bulk-select-cell';tr.insertBefore(td,tr.firstChild)}td.innerHTML=bulkMode?`<input class="bulk-row-checkbox" type="checkbox" data-bulk-id="${id}" ${selected.has(id)?'checked':''}>`:'';tr.classList.toggle('bulk-selection-mode',bulkMode)});const th=q('#applicants table thead tr');if(th){let cell=th.querySelector('.bulk-head-cell');if(bulkMode&&!cell){cell=document.createElement('th');cell.className='bulk-head-cell';cell.textContent='선택';th.insertBefore(cell,th.firstChild)}else if(!bulkMode&&cell)cell.remove()}}
function ensureBulkToggle(){ bulkMode=true; decorateRows(); }
