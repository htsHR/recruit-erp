...
function runSearch(){
 ...
 .filter(a=>!kw||[a.name,a.phone,a.school,a.workplace,a.status,ms[a.id]].join(' ').toLowerCase().includes(kw)); window.__erpAdvancedFilterIds=advancedResults.map(a=>a.id); window.__erpAdvancedCriteria={...c}; renderAdvanced(); if(typeof renderTable==='function')renderTable();
}
function renderAdvanced(){const ms=managers();if(!el('asSummary')||!el('asResultList')){updateBulkDock();return;}el('asSummary').textContent=`총 ${advancedResults.length}명 · 선택 ${[...selected].filter(id=>advancedResults.some(a=>a.id===id)).length}명`;el('asResultList').innerHTML=advancedResults.length?advancedResults.map(a=>`<label class="advanced-result-item ${selected.has(a.id)?'selected':''}"><input type="checkbox" data-advanced-id="${a.id}" ${selected.has(a.id)?'checked':''}><span class="advanced-result-copy"><strong>${safe(a.name||'이름없음')} · ${safe(a.status||'')}</strong><small>${safe(a.phone||'연락처 없음')} · ${safe(a.school||'학교 미입력')} · ${safe(a.workplace||'근무지 미입력')}</small></span><span class="advanced-result-meta">지원 ${safe(a.applyDate||'-')}<br>면접 ${safe(a.interviewDate||'-')}<br>${ms[a.id]?`담당 ${safe(ms[a.id])}`:'담당자 미지정'}</span></label>`).join(''):'<div class="empty">조건에 맞는 지원자가 없습니다.</div>';updateBulkDock()}
function saved(){try{return JSON.parse(localStorage.getItem(SAVED_KEY)||'[]')}catch{return []}}
function renderSaved(){if(!el('asSavedList'))return;el('asSavedList').innerHTML=saved().map((x,i)=>`<span class="saved-search-chip"><button data-load-search="${i}">${safe(x.name)}</button><button title="삭제" data-delete-search="${i}">×</button></span>`).join('')||'<span class="muted">저장된 검색조건이 없습니다.</span>'}
...
function bind(){
 hydrateOptions();renderSaved();ensureBulkToggle();updateConditionCount();
 qa('#advancedSearch input,#advancedSearch select').forEach(x=>x.addEventListener('change',updateConditionCount)); if(el('asRun'))el('asRun').onclick=runSearch;if(el('asReset'))el('asReset').onclick=()=>{setCriteria({});advancedResults=[];window.__erpAdvancedFilterIds=null;window.__erpAdvancedCriteria=null;renderAdvanced();if(typeof renderTable==='function')renderTable()};if(el('asSave'))el('asSave').onclick=saveCurrent;
 if(el('asSavedList'))el('asSavedList').onclick=e=>{const l=e.target.closest('[data-load-search]'),d=e.target.closest('[data-delete-search]');const arr=saved();if(l){setCriteria(arr[+l.dataset.loadSearch].criteria);runSearch()}if(d){arr.splice(+d.dataset.deleteSearch,1);localStorage.setItem(SAVED_KEY,JSON.stringify(arr));renderSaved()}};
 const asResultList=el('asResultList'),asSelectAll=el('asSelectAll'),asGoBulk=el('asGoBulk');
 ...
 if(asSelectAll)asSelectAll.onclick=()=>{advancedResults.forEach(a=>selected.add(a.id));renderAdvanced()};
 if(asGoBulk)asGoBulk.onclick=openBulk;
 document.addEventListener('change',e=>{const c=e.target.closest('[data-bulk-id]');if(!c)return;c.checked?selected.add(c.dataset.bulkId):selected.delete(c.dataset.bulkId);updateBulkDock()});const bulkClear=el('bulkClear'),bulkOpen=el('bulkOpen'),bulkClose=el('bulkClose'),bulkBackdrop=el('bulkBackdrop');if(bulkClear)bulkClear.onclick=()=>{selected.clear();if(asResultList)renderAdvanced();updateBulkDock()};if(bulkOpen)bulkOpen.onclick=openBulk;if(bulkClose)bulkClose.onclick=closeBulk;if(bulkBackdrop)bulkBackdrop.onclick=closeBulk;
