...
function bind(){
 ...
 qa('#advancedSearch input,#advancedSearch select').forEach(x=>x.addEventListener('change',updateConditionCount)); el('asRun').onclick=runSearch;el('asReset').onclick=()=>{setCriteria({});advancedResults=[];window.__erpAdvancedFilterIds=null;window.__erpAdvancedCriteria=null;renderAdvanced();if(typeof renderTable==='function')renderTable()};el('asSave').onclick=saveCurrent;
