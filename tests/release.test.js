...
const changedAssets=['js/app-version.js','js/production-readiness.js'];
changedAssets.forEach(file=>assert.equal(assetVersions.get(file),version,`${file}의 v12.3.0 캐시 버전이 일치하지 않습니다.`));
assert.deepEqual(
  localAssets.filter(([,file,assetVersion])=>assetVersion===version).map(([,file])=>file).sort(),
  [...changedAssets].sort(),
  'v12.3.0에서 수정된 JS만 새 캐시 버전을 사용해야 합니다.'
);
