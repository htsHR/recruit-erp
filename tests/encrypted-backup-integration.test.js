...
assert.doesNotMatch(ui,/panel\.id='bcEncryptedPanel'/);assert.doesNotMatch(ui,/openDialog\(\{kind:'export'/);assert.match(ui,/exportPlain/);
...
assert.ok(index.indexOf('js/encrypted-backup.js')<index.indexOf('js/backup-center.js'));assert.ok(index.indexOf('js/backup-center.js')<index.indexOf('js/encrypted-backup-ui.js'));assert.match(index,/accept="\.erpbackup,\.json,application\/json"/);assert.match(index,/css\/encrypted-backup\.css\?v=12\.5\.3/);
assert.match(workflow,/UI_SCREENSHOT_DIR: artifacts\/ui-v12\.5\.3/);assert.match(workflow,/path: artifacts\/ui-v12\.5\.3/);
