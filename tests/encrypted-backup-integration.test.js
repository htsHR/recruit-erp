...
assert.ok(index.indexOf('js/encrypted-backup.js')<index.indexOf('js/backup-center.js'));assert.ok(index.indexOf('js/backup-center.js')<index.indexOf('js/encrypted-backup-ui.js'));assert.match(index,/accept="\.erpbackup,\.json,application\/json"/);assert.match(index,/css\/encrypted-backup\.css\?v=10\.61\.0/);
assert.match(workflow,/UI_SCREENSHOT_DIR: artifacts\/ui-v10\.61\.0/);assert.match(workflow,/path: artifacts\/ui-v10\.61\.0/);
const supabaseFiles=fs.readdirSync(path.join(root,'supabase'),{recursive:true}).map(String);assert.ok(!supabaseFiles.some(file=>file.includes('10.61')||file.includes('v10_61')),'v10.61.0은 Supabase migration을 추가하지 않습니다.');
assert.ok(fs.existsSync(path.join(root,'docs','ENCRYPTED_BACKUP_v10.60.0.md')));assert.ok(fs.existsSync(path.join(root,'CHANGELOG_v10.61.0.md')));
