...
// 5~6. 브라우저 저장/새로고침과 재가져오기 직렬화 경계에서도 값이 합쳐지지 않습니다.
const reloaded=JSON.parse(JSON.stringify(different));assert.equal(reloaded.position,'파트장');assert.equal(reloaded.role,'품질검사');
const localRoundTrip=plan({...reloaded,id:'local-row'},record('파트장','품질검사'));assert.equal(localRoundTrip.position,'파트장');assert.equal(localRoundTrip.role,'품질검사');
...
assert.doesNotMatch(employeesSource,/supabase|employeeCloudRow|EMPLOYEE_CLOUD_FIELDS/i,'사원 저장 경로에 폐기된 원격 변환이 남으면 안 됩니다.');
...
const retiredDir=path.join(root,'supabase');const retiredFiles=fs.existsSync(retiredDir)?fs.readdirSync(retiredDir,{recursive:true}).filter(file=>fs.statSync(path.join(retiredDir,file)).isFile()):[];
assert.deepEqual(retiredFiles,[],'폐기된 원격 migration 파일이 남으면 안 됩니다.');
