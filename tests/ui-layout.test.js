...
['btnRosterOrderEdit','rosterOrderEditor','rosterOrderEditorList','btnRosterOrderTimeSort','btnRosterOrderCancel','btnRosterOrderSave'].forEach(id=>assert.match(index,new RegExp(`id="${id}"`),`${id} 순서 편집 UI가 없습니다.`));
...
assert.match(index,/<h1>Recruit ERP<\/h1>/);assert.match(index,/<p>v12\.2\.0<\/p>/);assert.doesNotMatch(index,/\bPreview\b|VERSION 2\.0|TODAY WORK OPERATIONS · ACTION FIRST/);assert.doesNotMatch(applicants,/QUICK REVIEW/);
