...
채용 지원자, 일정, 사원명부와 협력학교를 관리하는 웹 ERP입니다. 현재 운영 버전은 **v12.0.1**입니다.

운영 홈페이지: https://recruit-erp.vercel.app

## 매일 사용할 때

1. 화면의 `LOCAL ONLY` 또는 `이 브라우저 저장` 표시를 확인합니다.
2. 지원자·사원·학교·일정은 기존 브라우저 저장 기능으로 사용합니다.
3. 회사 공용폴더를 사용하는 PC에서는 Bridge가 연결되었는지만 확인합니다.
4. 중요한 대량 수정 전에는 `백업/내보내기`에서 암호화 전체 백업을 받습니다.
...
- 로그인 UI 0건, 초기 홈 즉시 표시, 기존 세션 무시와 LOCAL ONLY Supabase 조회·저장·동기화 0회
- 첫 페인트 전 밝은 부트 가드와 1280·1366의 100%·125% 첫 진입·새로고침·직접 라우트 구형 셸 0프레임
...
로컬 UI 스크린샷은 기본적으로 `artifacts/ui-v12.0.1`에 저장됩니다. 테스트 데이터는 이름·연락처·학교가 모두 가상인 전용 자료만 사용합니다.
...
- 이메일·비밀번호·로그인 선택·로그아웃·Supabase 계정 UI를 화면과 접근성 트리에서 제거했습니다.
- 앱은 시작과 동시에 `LOCAL ONLY`로 열리며, 과거 인증 세션이 남아 있어도 Supabase 클라이언트·세션 조회·클라우드 자동 동기화를 시작하지 않습니다.
- 지원자·사원·학교·일정의 기존 localStorage 키와 저장 함수, 공용폴더 Bridge, 권한 및 저장 실패 원상복구는 변경하지 않았습니다.
- 첫 페인트 전에 밝은 중립 부트 가드를 적용해 v12 셸 준비 전 구형 짙은 사이드바와 업무 자료가 보이지 않게 했습니다.
- Supabase CDN, `supabase_config.js`, `auth-init.js`는 첫 화면 참조에서만 분리했으며 파일과 클라우드 데이터 계층은 향후 인증 복구를 위해 보존합니다.
- Supabase 원격 데이터·인증 계정·DB·RLS·migration은 변경하지 않았습니다.

자세한 변경 내역은 `CHANGELOG_v12.0.1.md`를 확인하세요.

## v12.0.0 UX Reboot Phase 1 — Greeting Hybrid

- 전역 앱 셸을 밝은 운영형 구조로 재구성하고 기존 화면을 오늘, 지원자, 일정·연락, 입사·직원, 학교·채용채널, 분석·관리의 6개 그룹에 연결했습니다.
- 1280·1366 데스크톱의 100%와 125% 확대에서 밝은 확장 메뉴와 72px 아이콘 레일, 150ms 임시 오버레이 확장, 500ms 복귀, 핀 고정을 유지합니다. 새 UI 환경설정 키는 만들지 않습니다.
...
- OWNER 1차 시각검수에 따라 브랜드를 `Recruit ERP · v12.0.0 Preview`로 통일하고, `내 보기` 선택 글자 잘림과 비차단 공용저장 안내의 과도한 강조를 줄였습니다.
