/* Recruit ERP v12.0.2 LOCAL ONLY storage capacity and IndexedDB safety mirror. */
(function(root,factory){
  ...
  function init(){
    ...
    Promise.resolve(root.erpStateReady).then(state=>{
      if(!state?.ok)return;
  ...
  const api={VERSION,DB_NAME,DB_VERSION,DATASET_STORE,SNAPSHOT_STORE,SNAPSHOT_META_STORE,MAX_SNAPSHOTS,LOCAL_WARNING_BYTES,LOW_REMAINING_BYTES,DATASETS,byteLength,formatBytes,shouldWarn,datasetSummary,safeParseDataset,openDb,mirrorDataset,mirrorAll,listSnapshots,saveSnapshot,deleteSnapshot,createSnapshot,localUsage,estimateStorage,render,ensureUi,init};
