...
(function(root){
  ...
  function installPanel(){
    ...
    panel.innerHTML=`<div class="backup-section-head"><div><p class="eyebrow">AES-GCM · PASSWORD PROTECTED</p><h3>암호화 백업</h3><p>개인정보가 포함된 백업은 비밀번호로 잠가 .erpbackup 파일로 저장합니다.</p></div><span class="backup-section-number">🔒</span></div><div class="encrypted-support ${core?.isSupported()?'ok':'error'}"><strong>${core?.isSupported()?'암호화 기능 사용 가능':'암호화 기능 사용 불가'}</strong><span>${supportMessage()}</span></div><div class="backup-primary-action encrypted-primary"><div><h4>암호화 전체 백업</h4><p>현재 핵심업무 데이터와 이전 버전의 보존 데이터까지 하나의 파일에 안전하게 담습니다.</p></div><button class="primary" id="bcEncryptedFull" type="button">전체 백업</button></div><div class="backup-component-grid encrypted-components"><div class="backup-component-card"><strong>지원자만 백업</strong><span>지원자 데이터만 별도로 보관할 때 사용합니다.</span><button class="mini" data-encrypted-backup="applicants" type="button">지원자 백업</button></div></div><ul class="encrypted-backup-guidance"><li>이전 학교·사원·입사대기·문구 데이터는 전체 백업에 그대로 보존됩니다.</li><li>비밀번호는 서버나 브라우저 저장소에 저장되지 않습니다.</li><li>비밀번호를 잊으면 백업을 복구할 수 없습니다.</li></ul>`;
