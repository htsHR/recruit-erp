/* Recruit ERP v12.0.2 LOCAL ONLY production readiness. */
(function(root,factory){
  ...
  async function encryptionRoundTrip(){
    try{
      ...
      const bytes=new TextEncoder().encode('Recruit ERP v12.0.2 synthetic readiness');
      ...
      return new TextDecoder().decode(plain)==='Recruit ERP v12.0.2 synthetic readiness';
    }catch{return false;}
  }
  ...
  async function automaticChecks(evidence=context()){
    ...
    const versionMatch=!versions.length||versions.every(value=>value===VERSION);
    ...
    return [
      {id:'version',testType:'기능 시험',label:'v12.0.2 버전 일치',status:versionMatch?'pass':'fail',detail:versionMatch?'v12.0.2 화면·브랜드·정적 파일 버전이 일치합니다.':'정적 파일 버전이 일치하지 않습니다.'},
