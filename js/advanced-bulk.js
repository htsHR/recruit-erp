...
const saveManagers=x=>safeLocalStorageSet(MANAGER_KEY,JSON.stringify(x));
...
function applyBulk(){if(window.erpPermissions&&!window.erpPermissions.require('applicant.write'))return;const rows=selectedApplicants(),f=el('bulkField').value,v=bulkValue();if(!rows.length||!v)return alert('대상과 변경 값을 확인해주세요.');if(!el('bulkConfirmCheck').checked)return;if(!confirm(`${rows.length}명의 ${f==='status'?'상태':f==='manager'?'담당자':'근무지'}를 "${v}"(으)로 변경할까요?`))return;const before=applicants;if(f==='manager'){const ms=managers();rows.forEach(a=>ms[a.id]=v);if(!saveManagers(ms)||!save())return;}else{applicants=applicants.map(a=>selected.has(a.id)?{...a,[f]:v,updatedAt:new Date().toISOString()}:a);if(!save()){applicants=before;return;}}closeBulk();hydrateOptions();runSearch();alert(`${rows.length}명 일괄 변경을 완료했습니다.`)}
function csv(){const rows=selectedApplicants(),ms=managers();const cols=['성명','연락처','상태','근무지','학교','지원일','면접일','입사일','담당자'];const lines=[cols.map(v=>window.erpSafety.csvCell(v,true)).join(','),...rows.map(a=>[a.name,a.phone,a.status,a.workplace,a.school,a.applyDate,a.interviewDate,a.hireDate,ms[a.id]||''].map(v=>window.erpSafety.csvCell(v,true)).join(','))];dl(`지원자_선택_${rows.length}명_${new Date().toISOString().slice(0,10)}.csv`,lines.join('\n'),'text/csv;charset=utf-8')}
