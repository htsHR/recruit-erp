...
updateStorageNote=function(){
  ...
  el.className='security-note local-master-note';
  el.innerHTML='<strong>회사 로컬 운영 모드</strong><span>현재 브라우저의 데이터가 업무 기준입니다. 퇴근 전 JSON 백업을 확인하세요.</span>';
};
...
try{ uxInit(); }catch(err){ console.error('Recruit ERP COMPLETE_UX init error',err); }
})();
