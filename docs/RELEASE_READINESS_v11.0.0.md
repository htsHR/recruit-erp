...
2. `applicants`, `employees`, `schools`, `applicant_snapshots`, `user_roles`, `audit_logs`에 RLS가 켜져 있는지 확인합니다.
3. 공개 `SECURITY DEFINER` 함수의 `anon`·`authenticated` 직접 실행 경고가 사라졌는지 [Security Advisor](https://supabase.com/docs/guides/database/database-linter)에서 확인합니다.
4. `app_settings`와 `allowed_users`의 RLS 성능 경고가 사라졌는지 확인합니다.
5. Supabase Auth의 [유출 비밀번호 보호](https://supabase.com/docs/guides/auth/password-security)를 켭니다. 이 설정은 저장소 코드나 SQL migration으로 대신하지 않습니다.
6. 관리자·채용담당자·조회전용 실제 테스트 계정으로 허용·차단 작업을 확인합니다.
...
운영 준비 화면의 `점검 결과 저장`은 항목 ID, 통과 상태, 시각, 역할과 가상 성능 건수만 JSON으로 저장합니다. 이름·연락처·주소·주민등록번호·메모·원본 업무 자료는 포함하지 않습니다.
